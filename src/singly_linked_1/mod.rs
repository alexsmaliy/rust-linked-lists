...
impl<'a, T> super::traits::List<'a, T> for List<T> {
    ...
    fn get(&'a self, ind: usize) -> &'a T {
        let mut count = 0;
        let mut optbox = self.maybe_contents.as_ref();
        while let Some(boxnode) = optbox {
            if count == ind {
                return &boxnode.value;
            }
            optbox = boxnode.maybe_next.as_ref();
            count += 1;
        }
        panic!("Error: index {} is out of bounds!", ind);
    }

    fn get_mut(&'a mut self, ind: usize) -> &'a mut T {
        let mut count = 0;
        let mut optbox = self.maybe_contents.as_mut();
        while let Some(boxnode) = optbox {
            if count == ind {
                return &mut boxnode.value;
            }
            optbox = boxnode.maybe_next.as_mut();
            count += 1;
        }
        panic!("Error: index {} is out of bounds!", ind);
    }
